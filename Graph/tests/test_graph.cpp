...
TEST_F(GraphTest, DfsDownward) {
    ...
    graph.DfsDownwardNonRec(handleA, [&paths](std::vector<NodeHandle> visited) {
    ...
    // EXPECT_EQ(paths[0], std::vector<NodeHandle>({handle1, handle2, handle3}));
}
